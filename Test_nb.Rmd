...
```{r}
draft_data<- drafts(draft_years = 1985:2021, nest_data = F)
```

## getting teams dictionary from BREF (Not needed)
```{r}
...
```{r}
comprehensive_stats<- merge(x=comprehensive_stats, y=dataBREFPlayersSalaries_grp, by="slugPlayerSeason", all=F) # Success! 8490
```
...
```{r}
comprehensive_stats<- comprehensive_stats %>% select(!(countTeamsPlayerSeasonPerMinute:`idPlayerNBA.y`))
```

## Dropping extra info columns-Pt2
```{r}
comprehensive_stats<- comprehensive_stats %>% select(!(countTeamsPlayerSeason:idPlayerNBA))
```

## Dropping the columns of minutesPerMinute and minutesTotals
```{r}
comprehensive_stats<- comprehensive_stats %>% select(!c(minutesPerMinute, minutesTotals))# 1 column dropped
```
...
count_NAs # 29 NAs in salary column, remove those rows
```

## drop 29 columns with NA salary values in them
```{r}
...
TODO:- REMOVE IRRELEVANT COLUMNS FROM comprehensive_stats, Normalise Salary data by salary cap by year, create the feature matrix X, and label set y by separating totSalary from comprehensive_stats into separate vector.
...
```{r}
salary_cap_data<- salary_cap_data %>% tidyr::separate(col=yearSeason, into=c("yearSeason", NA), sep='-')
```
...
```{r}
comprehensive_stats<- comprehensive_stats %>% dplyr::mutate(normalised_salary = totSalary / as.numeric(salary_cap))
```

## TODO:- fix the year season splitting, and the then look at methods to perform ensemble methods/blending in R
