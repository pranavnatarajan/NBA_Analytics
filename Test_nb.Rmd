...
player_stats<- bref_players_stats(seasons = 1985:2021, 
                                  tables = c("per_game", "advanced", "per_minute"),
                                  return_message = T, join_data = T, nest_data = F)
```
...
```{r}
player_stats<- player_stats %>% select(!starts_with("url"))# 80 variables currently
```

## Sanity Check:- checking number of distinct values in the slugplayerseason column, as that is our primary key
```{r}
nums_pl<- player_stats %>% count(slugPlayerSeason) %>% arrange(desc(n)) %>% head()
...
player_stats<- player_stats %>% distinct(slugPlayerSeason, .keep_all = T)# nrow = 16150
```
...
```{r}
player_stats<- merge(x=player_stats, y=dataBREFPlayersSalaries_grp, by="slugPlayerSeason", all=F) # Success! 9684
```
...
```{r}
player_stats<- player_stats %>% select(!tidyselect::starts_with("countTeamsPlayerSeason"))# 3 columns dropped
```

## Dropping extra info columns- Pt1
```{r}
player_stats<- player_stats %>% select(!c("minutesPerGame", "minutesPerMinute","isSeasonCurrent")) # 75 variables currently
```

<!-- ## Dropping extra info columns-Pt2 -->
<!-- ```{r} -->
<!-- player_stats<- player_stats %>% select(!(countTeamsPlayerSeason:idPlayerNBA))# 76 variables currently -->
<!-- ``` -->
...
```{r}
count_NAs<- apply(X=player_stats, MARGIN=2,
...
```{r}
player_stats<- player_stats %>% drop_na(totSalary) # 9628 tuples left
```


## Counting the number of duplicate primary keys (if any) in comprehensive_stats
```{r}
nums_pl<- player_stats %>% count(slugPlayerSeason) %>% arrange(desc(n))
...
## adding 1 to salary cap yearSeason for easy merging
```{r}
...
```{r}
player_stats<- merge(x=player_stats, y=salary_cap_data, by="yearSeason")
```

## mutating a new column as the ratio between salary and salary_cap, called normalised_salary
```{r}
player_stats<- player_stats %>% dplyr::mutate(normalised_salary = totSalary / as.numeric(salary_cap)) # WORKS. do not change anything else.
```
